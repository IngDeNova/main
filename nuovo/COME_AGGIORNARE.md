...
5. Apri `nuovo/contenuti.js`, clicca la matita, trova il progetto e cambia `foto: 18` in `foto: 20`. **Commit changes**.
...
4. Apri `nuovo/contenuti.js` → matita. Dentro `PROGETTI = [ … ]` copia un blocco esistente e incollalo dove vuoi che compaia (l'ordine nel file è l'ordine sul sito). Compila:
...
## Quando vorrai sostituire il vecchio sito con questo

Oggi il nuovo sito vive in `nuovo/` ed è raggiungibile su `ingdenova.github.io/main/nuovo/`.
Per portarlo all'indirizzo principale:

1. Sposta nella cartella principale del repository i file `index.html`, `stile.css`, `app.js`, `contenuti.js`, la cartella `assets/` e questa guida (sovrascrivendo il vecchio `index.html`).
2. In `contenuti.js` cambia la prima impostazione: `cartellaImmagini: "docs/"` (oggi è `"../docs/"`).
3. Cancella i file del vecchio sito e le pagine estranee (elenco qui sotto).
4. Cancella la cartella `nuovo/`.

File del vecchio sito da cancellare:
`arctecll.html, bwsc.html, comarcl.html, comarcll.html, comarclll.html, comarcpar.html, disarcll.html, disautrildig.html, ict.html, irf.html, lic.html, lpu.html, piaterurb.html, reccon.html, resarc.html, teccosacc.html, teccoscls.html, tecurb.html, tttcte.html, tttlog.html, tttpvo.html, tttred.html`, la cartella `images/` (sono i segnaposto grigi del vecchio tema), la vecchia cartella `assets/` del tema, `LICENSE.txt` e `README.txt` del tema.

Pagine estranee al portfolio, da cancellare quando vuoi (non sono collegate da nessuna parte):
`Unable_lco.html, WaveWater_istituzionale.html, etnarebels.html, evaluation.html, sibaco.html, sibama.html, sonuco.html, takticalcampetna.html, uth.html, uth_bis.html`.

Da **non** cancellare: `google2d904c41fdf14409.html` (verifica del sito per Google) e la cartella `docs/` (le foto).
...
- I font Barlow e Barlow Condensed sono inclusi in `assets/font/` (licenza SIL Open Font License, file incluso). Nessuna richiesta verso servizi esterni.
